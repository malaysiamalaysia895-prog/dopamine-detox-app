...
// ─── Creature Throw ───────────────────────────────────────────────────────────
// Tracks a Data Kraken thrown item on the grid with a 5-second countdown.

class CreatureThrow {
  final int col;
  final int row;
  final int itemId;
  final int secondsLeft; // countdown 5 → 0
  const CreatureThrow(this.col, this.row, this.itemId, this.secondsLeft);
  CreatureThrow withSeconds(int s) => CreatureThrow(col, row, itemId, s);
}
