            ...
            child: SafeArea(
              child: Stack(
                ...
                children: [
                  ...
                  // Data Kraken creature boss (L23, L25, L27, L29)
                  CreatureOverlay(
                    controller: ref.read(gameProvider.notifier).creatureController,
                  ),
                  // Alien Boss (L31, L32, L33) — sits in top of game board area
                  AlienOverlay(
                    controller: ref.read(gameProvider.notifier).alienController,
                    ...
                    getSpawnerCenter: () {
                      final spawnerBox = _spawnerKey.currentContext?.findRenderObject() as RenderBox?;
                  ...
                  NexusCoreOverlay(
                    controller: ref.read(gameProvider.notifier).nexusCoreController,
                    isDialogActive: ref.watch(dialogProvider) != ActiveDialog.none,
                    getCellRect: (col, row) {
                      final box      = _gridKey.currentContext?.findRenderObject() as RenderBox?;
                      final stackBox = _boardStackKey.currentContext?.findRenderObject() as RenderBox?;
                            ...
                            builder: (_, __) {
                              ...
                              return DragTarget<_DragData>(
                                onWillAcceptWithDetails: (details) =>
                                    rc.phase == RobotPhase.active &&
                                    details.data.itemId >= levelDef.spawnerItemId + 3,
                                onAcceptWithDetails: (details) {
                                  ref.read(gameProvider.notifier).deliverToRobot(
                                    details.data.col, details.data.row);
                                },
                                builder: (ctx, candidates, _) => Padding(
                                  padding: const EdgeInsets.only(right: 4),
                                  child: _RobotQuotaChip(
                                    ctrl: rc,
                                    spawnerItemId: levelDef.spawnerItemId + 3,
                                    isTargeted: candidates.isNotEmpty,
                                  ),
                                ),
                              );
                            },
...
class _RobotQuotaChip extends StatelessWidget {
  ...
  final bool isTargeted;   // true when an item is being dragged over this chip
  const _RobotQuotaChip({required this.ctrl, required this.spawnerItemId, this.isTargeted = false});
  @override
  Widget build(BuildContext context) {
    final done  = ctrl.mergesDone >= ctrl.mergesRequired && ctrl.mergesRequired > 0;
    final color = isTargeted
        ? const Color(0xFFFFD600)            // yellow glow when item dragged over
        : done ? const Color(0xFF00E676) : const Color(0xFFFF1744);
    return AnimatedScale(
      scale: isTargeted ? 1.10 : 1.0,
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: isTargeted
              ? [const Color(0xFF2A2200), const Color(0xFF1A1500)]
              : done
                  ? [const Color(0xFF0D2A1A), const Color(0xFF071510)]
                  : [const Color(0xFF2A0808), const Color(0xFF180505)],
        ),
        border: Border.all(color: color.withOpacity(isTargeted ? 1.0 : 0.75),
            width: isTargeted ? 2.5 : (done ? 2 : 1.5)),
        boxShadow: [BoxShadow(color: color.withOpacity(isTargeted ? 0.65 : 0.35),
            blurRadius: isTargeted ? 18 : 10)],
      ),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        SizedBox(
          width: 36, height: 32,
        ...
        const SizedBox(height: 1),
        Text(isTargeted ? 'DROP!' : 'Defeat', style: TextStyle(
          color: isTargeted ? const Color(0xFFFFD600) : Colors.white.withOpacity(0.80),
          fontSize: 8, letterSpacing: 0.3, fontWeight: isTargeted ? FontWeight.w900 : FontWeight.normal,
        )),
      ]),
    )); // close AnimatedScale
  }
...
class _GridCellState extends ConsumerState<_GridCell>
  ...
  Widget build(BuildContext context) {
    ...
    // Creature throw on this cell?
    final creatureThrows  = ref.watch(creatureThrowsProvider);
    final myCreatureThrow = creatureThrows
        .where((t) => t.col == widget.col && t.row == widget.row)
        .firstOrNull;
    ...
    return Stack(
      ...
      children: [
        core,
        // Data Kraken throw countdown ring
        if (myCreatureThrow != null)
          Positioned.fill(
            child: IgnorePointer(
