...
class RobotController extends ChangeNotifier with WidgetsBindingObserver {
  ...
  void triggerForLevel(int level, {required VoidCallback onClearGrid}) {
    ...
    // Mechanical entry impact — triple-burst haptic so player always feels it
    ...
    _safeVibrate(pattern: [0, 200, 80, 200, 80, 500]);
  ...
  // async so we can await hasVibrator() before calling vibrate —
  // skipping this check causes silent failures on many Android devices.
  Future<void> _safeVibrate({List<int>? pattern, int duration = 400}) async {
    if (!_appInForeground) return;
    try {
      final bool has = await Vibration.hasVibrator() ?? false;
      if (!has) return;
